...
def records(manifest: Path | None = None) -> list[object]:
    if manifest is not None:
        return load_manifest_records(manifest, source_catalog=SourceCatalog.EPO)
    return records_for_source(SourceCatalog.EPO)
...
def main() -> None:
    ...
    print(f"wrote {len(items)} EPO metadata records to {args.output}")
